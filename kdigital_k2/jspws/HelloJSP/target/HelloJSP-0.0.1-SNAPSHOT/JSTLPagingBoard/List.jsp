...
<c:set var="boardLists" value="<%=boardLists %>" />
	<c:if test="${session.UserId} != null &&  ${session.UserId eq 'musthave' }">
		<th width="15%">수정</th>
		<th width="15%">삭제</th>
		</c:if>
<%//} %>
...
%>
	<tr align="center">
		<td><%=virtualNum %>(${dto.num})</td><!-- 게시물 번호 -->
		<td align="left"> <!-- 제목(+하이퍼링크) -->
			<a href="View.jsp?num=${dto.num}">${dto.title}</a>
		</td>
		<td align="center">${dto.id}</td> <!-- 작성자 아이디 -->
		<td align="center">${dto.visitcount}</td> <!-- 조회수 -->
		<td align="center">${dto.postdate}</td> <!-- 작성일 -->
		
<c:if test="${session.UserId} != null &&  ${session.UserId eq 'musthave' }">
		<td > <button type="button" onclick="location.href='Edit.jsp?num=${dto.num}'" > 수정</button></td>
		<td ><button type="button" onclick="deletePost(${dto.num})" >삭제</button> </td>
</c:if>
